...
from cares_rl_statistics.statistics import holm_correction, probability_of_improvement, signed_cliffs_delta
...
def test_constant_sample_has_exact_bootstrap_interval():
    ...
    assert bootstrap_bca_ci(
        values, np.mean, 100, 0.95, np.random.default_rng(0)
    ) == (4.0, 4.0)
