...
def create_DynaSAC_SABR(observation_size, action_num, config: AlgorithmConfig):
    """
    Create networks for model-based SAC agent. The Actor and Critic is same.
    An extra world model is added.
...
def create_DynaSAC_ScaleBatchReweight(observation_size, action_num, config: AlgorithmConfig):
    ...
    agent = DynaSAC_ScaleBatchReweight(
        actor_network=actor,
        critic_network=critic,
        world_network=world_model,
        actor_lr=config.actor_lr,
...
def create_DynaSAC_NormalizedSigmoidBatchReweight(observation_size, action_num, config: AlgorithmConfig):
    """
    Create networks for model-based SAC agent. The Actor and Critic is same.
    An extra world model is added.

    """
    from cares_reinforcement_learning.algorithm.mbrl import DynaSAC_NormalizedSigmoidBatchReweight
    from cares_reinforcement_learning.networks.SAC import Actor, Critic
    from cares_reinforcement_learning.networks.world_models import EnsembleWorldAndOneReward
    ...
    agent = DynaSAC_NormalizedSigmoidBatchReweight(
        actor_network=actor,
        critic_network=critic,
        world_network=world_model,
        actor_lr=config.actor_lr,
