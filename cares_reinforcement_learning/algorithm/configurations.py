...
class LSDConfig(SACConfig):
    ...
    is_using_buffer:bool = False # if false: update networks using only last {number_steps_per_train_policy} transitions collected. i.e. clear buffer every {'num per train'} steps.

    batch_size:int = 2000
    number_steps_per_train_policy:int = batch_size # consider as number of steps per batch for training, "epoch" in original implementation
    minibatch_size:int = 1024 # size of minibatch to update networks 
    
    agent_loops_per_batch:int = 1 # how many times to loop over batch data for RL agent updates.  "max_optimization_epochs" in original
    agent_num_minibatches_per_batch: int = 64 # how many times to update weights per minibatch, "trans_optimization_epochs" in original

    encoder_loops_per_batch:int = 1 # how many times to loop over batch data for encoder updates,  "te_max_optimization_epochs" in original
    encoder_num_minibatches_per_batch: int = 32 # how many times to update encoder weights per minibatch, "te_trans_optimization_epochs" in original
    ...
    encoder_config: MLPConfig = MLPConfig (
        layers=[
            ...
            FunctionLayer(layer_type="ReLU"),
            TrainableLayer(layer_type="SpectralNormLinear",in_features=512,out_features=512),
            FunctionLayer(layer_type="ReLU"),
