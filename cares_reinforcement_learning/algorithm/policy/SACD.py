...
class SACD(SAC):
    ...
    def act(
        ...
        return ActionSample(action=action.cpu(), source="policy", extras={"entropy": self.entropy.item() if self.entropy is not None else None})
    ...
    def train(
        ...
        if hasattr(self, "autoencoder"):
            if hasattr(observation_tensor, "image"):
                ae_loss = self._update_autoencoder(observation_tensor["image"])
                info["ae_loss"] = ae_loss
            else:
                ae_loss = self._update_autoencoder(observation_tensor)
                info["ae_loss"] = ae_loss
    ...
    def _set_encoding(
        ...
        elif encoder_type == ImageEncoderType.VANILLA_AE:
