...
class PPO2SIL(VectorAlgorithm):
    ...
    def _calculate_rewards_to_go_episode_end(
        ...
        episode_ends = batch_episode_ends.cpu().numpy()
    ...
    def _discounted_cumulative_sum_gae(
        self, batch_deltas: torch.Tensor, batch_episode_ends: torch.Tensor
    ) -> torch.Tensor:
        dis_sum: list[float] = []
        discounted_sum = 0

        for d, e in zip(reversed(batch_deltas), reversed(batch_episode_ends)):
    ...
    def _GAE_calculator(
        ...
        advantages = self._discounted_cumulative_sum_gae(deltas, batch_episode_ends).squeeze() # shape 5000
    ...
    def train_policy(self, training_context: TrainingContext) -> dict[str, Any]:
        ...
        dones        = experiences[4]
        episode_ends  = experiences[5] if len(experiences) > 5 else dones
        ...
        rtgs_episode_end = self._calculate_rewards_to_go_episode_end(rewards_tensor, dones_tensor, episode_ends_tensor, next_states_tensor) # shape 5000
