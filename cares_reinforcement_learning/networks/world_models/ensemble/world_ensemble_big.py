...
class Ensemble_Dyna_Big(World_Model):
    ...
    def __init__(
        ...
        self.optimizers = [
            optim.Adam(self.world_models[i].parameters(), lr=l_r)
            for i in range(self.num_models)
        ]
        self.statistics = {}
    ...
    def pred_next_states(
        ...
        # Normalized
        predictions_means = denormalize_observation_delta(
            predictions_norm_means, self.statistics
        )
    ...
    def estimate_uncertainty(
        ...
        if train_reward:
            ...
            samples = torch.reshape(
                samples, (sample_times * self.num_models, self.observation_size)
            )
            samples = denormalize_observation_delta(samples, self.statistics)
            observationss = torch.repeat_interleave(
                observation, repeats=sample_times * self.num_models, dim=0
            )
            actionss = torch.repeat_interleave(
                actions, repeats=sample_times * self.num_models, dim=0
            )
            samples += observationss

            if self.sas:
                if self.prob_rwd:
                    rewards, rwd_var = self.reward_network(
                        observationss, actionss, samples
                    )
            ...
            next_state_samples = next_state_samples.reshape(
                (self.num_models * 20, self.observation_size)
            )
            next_state_samples = denormalize_observation_delta(
                next_state_samples, self.statistics
            )
    ...
    def train_together(
        ...
        actions = torch.repeat_interleave(
            act_s.unsqueeze(dim=0), repeats=sample_times, dim=0
        )
        states = torch.repeat_interleave(
            state_s.unsqueeze(dim=0), repeats=sample_times, dim=0
        )
        rwd_s = torch.repeat_interleave(
            rwd_s.unsqueeze(dim=0), repeats=sample_times, dim=0
        )

        samples = torch.reshape(
            samples, (samples.shape[0] * samples.shape[1], self.observation_size)
        )
        states = torch.reshape(
            states, (states.shape[0] * states.shape[1], states.shape[2])
        )
        actions = torch.reshape(
            actions, (actions.shape[0] * actions.shape[1], actions.shape[2])
        )
